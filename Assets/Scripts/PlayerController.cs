...
public class PlayerController : MonoBehaviour
{
    ...
    [Range(1.0f, 650.0f)] public float walkingSpeed = 140;
    ...
    public void OhPoopGotMe()
    {
        poopText.gameObject.SetActive(true);
        poopText.transform.DOScale(new Vector3(1.5f, 1.5f, 1.5f), 0.5f).SetEase(Ease.OutBounce).OnComplete(() =>
        {
            poopText.transform.DOScale(new Vector3(1f, 1f, 1f), 0.5f).SetEase(Ease.InBounce).OnComplete(() =>
            {
    ...
    [SerializeField] private MMF_Player poopMoveFeedback;
    ...
    void Update()
    {
        ...
        if (enableCameraControl)
        {
            ...
            maxCameraDistInternal =
                Mathf.Clamp(maxCameraDistInternal - (mouseScrollWheel * (cameraZoomSensitivity * 2)),
                    capsule.radius * 2, maxCameraDistance);
        }
        ...
        HeadRotDirForInput =
            Mathf.MoveTowardsAngle(HeadRotDirForInput, headRot.y, bodyCatchupSpeed * (1 + Time.deltaTime));
        MovInput_Smoothed =
            Vector2.MoveTowards(MovInput_Smoothed, MovInput, inputResponseFiltering * (1 + Time.deltaTime));

        InputDir = Quaternion.AngleAxis(HeadRotDirForInput, Vector3.up) *
    ...
    void RotateView(Vector2 yawPitchInput, float inputSensitivity, float cameraWeight)
    {
        ...
        headRot = Vector3.SmoothDamp(headRot, headRot + ((Vector3)yawPitchInput * (inputSensitivity * 5)),
            ref cameraPosVelRef, (Mathf.Pow(cameraWeight, 2)) * Time.fixedDeltaTime, maxDelta, Time.fixedDeltaTime);
    ...
    void UpdateBodyRotation_3rdPerson()
    {
        ...
        else if (!currentGroundInfo.isInContactWithGround && rotateCharacterToCameraForward)
        {
            transform.localRotation =
                (Quaternion.Euler(
                    Vector3.up * Mathf.MoveTowardsAngle(p_Rigidbody.rotation.eulerAngles.y, headRot.y, 10)));
        }
    }
    ...
    void MovePlayer(Vector3 Direction, float Speed)
    {
            ...
            else if ((currentGroundInfo.isInContactWithGround) && currentGroundInfo.groundAngle < hardSlopeLimit)
            {
                p_Rigidbody.velocity = (Vector3.MoveTowards(p_Rigidbody.velocity,
                    Vector3.ClampMagnitude(((Direction) * ((Speed) * Time.fixedDeltaTime)) + (Vector3.down),
                        Speed / 50), 1));
                poopMoveFeedback.PlayFeedbacks();
            }
        }
        ...
        else if (!currentGroundInfo.isInContactWithGround)
        {
            p_Rigidbody.AddForce(
                ...
                currentGroundInfo.groundAngleMultiplier_Inverse, ForceMode.Acceleration);
    ...
    void GetGroundInfo()
    {
        ...
        if (Jumped && (Physics.Raycast(transform.position, Vector3.down, (capsule.height / 2) + 0.1f, whatIsGround) ||
                       Physics.CheckSphere(
